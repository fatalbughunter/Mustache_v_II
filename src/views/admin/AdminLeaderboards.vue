<template>
    <div class="admin-leaderboards">
        <div class="leaderboards-list">
            ...
            <div class="list-content">
                <transition name="fade" mode="out-in">
                    <div v-if="adminLeaderboardList.data === null || adminLeaderboardList.loading === true" class="content-loading" key="loading">
                        <LoadingAnimation />
                    </div>
                    <div v-else-if="adminLeaderboardList.data.length > 0" class="content-list" key="data">
                    ...
                    <div v-else class="content-empty" key="empty">No leaderboards found.</div>
                </transition>
            </div>
            <div class="list-pagination">
                <button v-on:click="adminSetPage(adminLeaderboardList.page - 1)" class="button-prev" v-bind:disabled="adminLeaderboardList.page <= 1">
                ...
                <div class="pagination-info">
                    PAGE <span class="text-green-gradient">{{adminLeaderboardList.page}}</span> / {{Math.ceil(adminLeaderboardList.count / 12) <= 0 ? '1' : Math.ceil(adminLeaderboardList.count / 12)}}
                </div>
                <button v-on:click="adminSetPage(adminLeaderboardList.page + 1)" class="button-next" v-bind:disabled="adminLeaderboardList.page >= Math.ceil(adminLeaderboardList.count / 12)">
                    <div class="button-inner">
                        <svg width="12" height="11" viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="leaderboards-filters">
            <AdminFilterSearch />
            ...
            <div class="filters-create">
                ...
                <div class="create-prize">
                    ...
                    <AdminPrizeElement v-for="(prize, index) in adminPrizes" v-bind:key="index" v-bind:pos="index" v-bind:prize="prize" />
                </div>

                <button v-on:click="adminAddButton()" class="button-add">ADD PRIZE</button>
                <button v-on:click="adminCreateButton()" class="button-create">CREATE LEADERBOARD</button>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    import AdminFilterType from '@/components/admin/AdminFilterType';
    import AdminFilterDuration from '@/components/admin/AdminFilterDuration';
    import AdminLeaderboardElement from '@/components/admin/leaderboard/AdminLeaderboardElement';
    import AdminPrizeElement from '@/components/admin/leaderboard/AdminPrizeElement';
    ...
    export default {
        ...
        methods: {
            ...
            adminSetPage(page) {
                if(this.adminLeaderboardList.page === page) { return; }
                if(page < 1 || page > Math.ceil(this.adminLeaderboardList.count / 12)) { return; }

                this.adminSetLeaderboardListPage(page);

                const data = { page: this.adminUserList.page, search: this.adminFilterSearch };
                this.adminGetLeaderboardListSocket(data);
            },
            adminAddButton() {
            ...
            adminCreateButton() {
                ...
                const data = { type: this.adminFilterType.toLowerCase(), duration: this.adminFilterDuration, prizes: prizes };
                this.adminSendLeaderboardCreateSocket(data);
            }
        },
        computed: {
            ...mapGetters([
        ...
        created() {
            if(this.adminLeaderboardList.loading === false) {
                const data = { page: this.adminLeaderboardList.page, search: this.adminFilterSearch };
                this.adminGetLeaderboardListSocket(data);
            }
        },
...
<style scoped>
    ...
    .admin-leaderboards .leaderboards-list {
        width: calc(100% - 350px);
        padding-top: 15px;
        padding-right: 30px;
    }

    .admin-leaderboards .list-header {
        display: flex;
    ...
    .admin-leaderboards .header-type {
        width: 30%;
    }

    .admin-leaderboards .header-duration,
    .admin-leaderboards .header-state {
        width: 25%;
    }

    .admin-leaderboards .header-option {
        width: 20%;
        display: flex;
        justify-content: flex-end;
    }

    .admin-leaderboards .list-content {
        width: 100%;
        margin-top: 20px;
        padding: 20px 0;
    ...
    .admin-leaderboards .content-loading {
        width: 100%;
        height: 330px;
        display: flex;
    ...
    .admin-leaderboards .content-loading.fade-leave-active {
        transition: opacity 0.5s;
    }

    .admin-leaderboards .content-loading.fade-leave-to {
        opacity: 0;
    }
    ...
    .admin-leaderboards .content-empty {
        width: 100%;
        height: 192px;
        display: flex;
        justify-content: center;
    ...
    .admin-leaderboards .content-list.fade-enter-active,
    .admin-leaderboards .content-empty.fade-enter-active {
        transition: opacity 0.5s;
    }

    .admin-leaderboards .content-list.fade-enter-from,
    .admin-leaderboards .content-empty.fade-enter-from {
        opacity: 0;
    }

    .admin-leaderboards .list-pagination {
        width: 100%;
        display: flex;
        justify-content: space-between;
        align-items: center;
    ...
    .admin-leaderboards .list-pagination button {
        width: 52px;
        height: 37px;
        position: relative;
        padding: 1px;
    ...
    .admin-leaderboards .list-pagination button:disabled {
        cursor: not-allowed;
    }

    .admin-leaderboards .list-pagination button:before {
        content: '';
        width: 100%;
        height: 100%;
        position: absolute;
    ...
    .admin-leaderboards .list-pagination button:disabled:before {
        background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, #05253e 100%);
    }

    .admin-leaderboards .list-pagination button:after {
        content: '';
        width: calc(100% - 2px);
        height: calc(100% - 2px);
        position: absolute;
    ...
    .admin-leaderboards .list-pagination button .button-inner {
        width: 100%;
        height: 100%;
        display: flex;
        justify-content: center;
    ...
    .admin-leaderboards .list-pagination button:disabled .button-inner {
        background: var(--bg-blue-dark);
        box-shadow: inset 0px 2px 4px rgba(0, 0, 0, 0.35);
    }

    .admin-leaderboards .list-pagination button .button-inner svg {
        fill: #00ffc2;
    }

    .admin-leaderboards .list-pagination button:disabled .button-inner svg {
        fill: #7a93ac;
    }

    .admin-leaderboards .pagination-info {
        font-size: 12px;
        font-weight: 800;
        color: #5e768e;
    }

    .admin-leaderboards .leaderboards-filters {
        width: 350px;
    }

    .admin-leaderboards .filters-create {
        width: 100%;
        margin-top: 20px;
    }

    .admin-leaderboards .filters-create input {
        width: 100%;
        height: 47px;
        margin-top: 12px;
        padding: 0 15px;
    ...
    .admin-leaderboards .filters-create input::placeholder {
        color: #49687d;
    }

    .admin-leaderboards .create-prize {
        margin-top: 15px;
    }

    .admin-leaderboards .prize-title {
        font-size: 14px;
        font-weight: 700;
        color: #8bacc8;
    }

    .admin-leaderboards .filters-create button.button-add,
