    ...
    <div class="admin-stats">
        <div class="stats-list">
            <div class="list-header">
                ...
                <div class="header-npc">NPC</div>
                <div class="header-limiteds">LIMITEDS</div>
                <div class="header-robux">ROBUX</div>
                <div class="header-steam">STEAM</div>
                <div class="header-gift">GIFT CARDS</div>
                <div class="header-crypto">CRYPTO</div>
                <div class="header-cc">CC</div>
                <div class="header-profit">NET PROFIT</div>
            </div>
            <div class="list-content">
                <transition name="fade" mode="out-in">
                    <div v-if="adminStatsList.data === null || adminStatsList.loading === true" class="content-loading" key="loading">
                        <LoadingAnimation />
                    </div>
                    <div v-else-if="adminStatsList.data.length > 0" class="content-list" key="data">

                        <AdminStatsElement v-for="stat in adminStatsList.data" v-bind:key="stat._id" v-bind:stat="stat" />
            ...
            <div class="list-pagination">
                <button v-on:click="adminSetPage(adminStatsList.page - 1)" class="button-prev" v-bind:disabled="adminStatsList.page <= 1">
                ...
                <div class="pagination-info">
                    PAGE <span class="text-green-gradient">{{adminStatsList.page}}</span> / {{Math.ceil(adminStatsList.count / 12) <= 0 ? '1' : Math.ceil(adminStatsList.count / 12)}}
                </div>
                <button v-on:click="adminSetPage(adminStatsList.page + 1)" class="button-next" v-bind:disabled="adminStatsList.page >= Math.ceil(adminStatsList.count / 12)">
                    <div class="button-inner">
                        <svg width="12" height="11" viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            adminSetPage(page) {
                if(this.adminStatsList.page === page) { return; }
                if(page < 1 || page > Math.ceil(this.adminStatsList.count / 12)) { return; }

                this.adminSetStatsListPage(page);

                const data = { page: this.adminStatsList.page, search: this.adminFilterSearch };
...
<style scoped>
    ...
    .admin-stats .header-limiteds,
    .admin-stats .header-robux {
        width: 15%;
    }
    ...
    .admin-stats .list-pagination button:before {
        content: '';
        width: 100%;
        height: 100%;
        position: absolute;
    ...
    .admin-stats .list-pagination button:disabled:before {
        background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, #05253e 100%);
    }

    .admin-stats .list-pagination button:after {
        content: '';
        width: calc(100% - 2px);
        height: calc(100% - 2px);
        position: absolute;
